...
def get_daily_strategies(analysis_date):
    """Get the 3 strategies assigned for a specific date based on 5-day cycle"""
    strategy_list = list(STRATEGIES.keys())
    
    # Set August 9, 2025 as Day 1 of the cycle
    start_date = date(2025, 8, 9)
    
    # Calculate days since start date
    days_since_start = (analysis_date - start_date).days
    
    # Calculate cycle day (0-4) - Day 1 is cycle_day 0
    cycle_day = days_since_start % 5
    
    # Get 3 strategies for this cycle day
...
# -------------------------
# Estratégias (exatamente como você listou)
# -------------------------
STRATEGIES = {
 "Premium Stoch": [
    "Overview","VWAP AA","VWAP","Volume Delta","Stoch RSI","SMI","RSI(SMI)","RAINBOW_RSI"
 ],
 "LS Copy": [
    "Overview","NVT","RoC Bands","RoC","BBWP","PSO","RSI","RAINBOW_RSI"
 ],
 "PositionFlow": [
    "Overview","VWAP","Chart VWAP","MACZ VWAP","MFI","Fisher Transform","RAINBOW_RSI"
 ],
 "RenkoVol": [
    "Overview","GR-MMAs","Pi Cycle","Keltner & Bollinger","RSI Ichimoku","RWI",
    "BBWP","Trend Master","VWAP-RSI","ASO"
 ],
 "10h WWV Chart": [
    "Overview","BTC Log Regression","GR-MMAs","PiCycle","%R MA","RSI","Chaikin Oscillator",
    "Ultimate HODL Wave","PVT","BBWP"
 ],
 "Premium Osc Volatility": [
    "Overview","AO v2","ATR","RSI(ATR)","RSI","Stoch RSI"
 ],
 "RSI Strategy": [
    "Overview","Supertrend","CM_Ultimate","FibZia","EVWMA_LB","RSI(63)","%R MA","WT_X",
    "RAR [LuxAlgo]","RAINBOW_RSI","Coppock Curve","OBV","Global Liquidity Index"
 ],
 "WeisWaveVol": [
    "Overview","Bitcoin Rainbow Wave","CVD Candles","Volume Delta","RSI(Volume Delta)","CMF","RAINBOW_RSI"
 ],
 "PremiumACC": [
    "Overview","GC","PiCycle Top Indicator","EMA Ribbon","RCI3 Lines","TDIGM","CVD Ichimoku",
    "RSI(OSC)","RSI","Crypto Fear & Greed Index","BBWP"
 ],
 "VolPress": [
    "Overview","Alligator","GR-MMAs","CVO","WWV","RWI","MACD","TSI","Wick Delta Buy/Sell Pressure",
    "Elasticity","WT_LB"
 ],
 "Volatility": [
    "Overview","Symmetrical STD Channel","4 SMA","Golden Ratio Fib","RECON ATR","BBWP","SMI",
    "Stoch RSI","CA_Fisher","RSI","RSI line"
 ],
 "ACC/DIST": [
    "Overview","5 SMMA","Demand Index","BTC Transaction Fees","Ratings","BBWP","MVRV Z-Score"
 ],
 "LuxAlgo": [
    "Overview","Symmetrical STD Channel","Ultimate RSI","RWI"
 ],
 "Point and Figure": [
    "Overview","RW Monte Carlo","CM SuperGuppy","AOv2","BBWP","SNAB_RSI_EMA",
    "CM_Williams_Vix_Fix","ASO","RAINBOW_RSI"
 ],
 "Rational Strategy LT": [
    "Overview","MMBs","GR-Multiple MAs","SAR","Support and Resistance","Coppock Curve",
    "Stoch RSI","BBPower","%R MA","TRIX"
 ],
}
...
# Load existing data
# -------------------------
data = load_data()  # format: {strategy: {indicator: {note, status, tag, last_modified}}}

# Set all strategy tags to Neutral (including any that might be None)
...
start_date = date(2025, 8, 9)  # August 9, 2025 is Day 1
...
st.markdown("**Day {} of 5-day cycle** | Selected strategy: **{}** | Analysis date: **{}**".format(cycle_day, selected_strategy, analysis_date.strftime('%m/%d/%Y')))

# Progress indicator for today's strategies
...
# Form to edit notes for selected strategy
with st.form("notes_form"):
    
    # Single tag for entire strategy
    strategy_data = data.get(selected_strategy, {})
    # Get strategy tag from any existing indicator or use "Neutral" as default
    ...
    # Get strategy type from existing data or default to "Momentum"
    ...
    # For each indicator, an expander with fields
    ...
    if submitted:
        ...
        for ind in indicators:
            ...
            data[selected_strategy][ind] = {
                "note": note_val,
                "status": status_val,
                "momentum": strategy_type_val,  # Type applied to entire strategy
                "strategy_tag": strategy_tag_val,  # Tag applied to entire strategy
...
# Display panel for saved analyses (filterable)
st.subheader("📜 View saved analyses")

# Add option to view today's focus or all strategies
...
for strat in strategies_to_show:
    ...
    strategy_tag = ""
    if inds:
        first_indicator = next(iter(inds.values()))
        strategy_tag = first_indicator.get('strategy_tag', 'Neutral')
    ...
    for ind_name, meta in inds.items():
        momentum_type = meta.get("momentum", "Momentum")
...
st.info("**5-Day Cycle System**: Each day focuses on 3 strategies. Change the analysis date to see different strategy assignments. \
Tip: use the 'Export Analyses' buttons in the sidebar to download a backup.")
