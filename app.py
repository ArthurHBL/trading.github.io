...
def sanitize_key(s: str):
    return s.replace(" ", "_").replace("/", "_").replace("&", "and").replace("%", "pct").replace(".", "").replace("-", "_").replace("[", "").replace("]", "").replace("(", "").replace(")", "").replace(",", "_")
...
def generate_excel_report(data, analysis_date):
    ...
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        ...
        summary_data = {'Metric': ['Total Strategies', 'Total Indicators', 'Analysis Date'],
...
start_date = date(2025,8,9)
analysis_date = st.sidebar.date_input("Select analysis date:", value=date.today() if date.today()>=start_date else start_date, min_value=start_date)
...
with col1:
    st.download_button("📁 JSON", data=json.dumps(data,ensure_ascii=False,indent=2), file_name=f"strategy_analyses_{analysis_date.strftime('%Y%m%d')}.json")
with col2:
    if data:
        excel_buffer = generate_excel_report(data, analysis_date)
        st.download_button("📊 Excel", data=excel_buffer.getvalue(), file_name=f"trading_report_{analysis_date.strftime('%Y%m%d')}.xlsx")
...
# --- Form with improved visuals ---
with st.form("notes_form"):
    strategy_data = data.get(selected_strategy,{})
    
    # Strategy Tag
    current_strategy_tag = next((ind.get("strategy_tag") for ind in strategy_data.values() if "strategy_tag" in ind), "Neutral")
    tag_colors = {"Buy":"🟢 Buy","Sell":"🔴 Sell","Neutral":"⚪ Neutral"}
    strategy_tag = st.selectbox(f"Strategy '{selected_strategy}' tag:", options=["Neutral","Buy","Sell"], index=["Neutral","Buy","Sell"].index(current_strategy_tag if current_strategy_tag in ["Neutral","Buy","Sell"] else 0), format_func=lambda x: tag_colors[x])
    
    # Strategy Type
    current_strategy_type = next((ind.get("momentum") for ind in strategy_data.values() if "momentum" in ind), "Not Defined")
    type_colors = {"Momentum":"📈 Momentum","Extreme":"⚡ Extreme","Not Defined":"❔ Not Defined"}
    strategy_type = st.selectbox(f"Strategy '{selected_strategy}' type:", options=["Momentum","Extreme","Not Defined"], index=["Momentum","Extreme","Not Defined"].index(current_strategy_type if current_strategy_type in ["Momentum","Extreme","Not Defined"] else 2), format_func=lambda x: type_colors[x])
    ...
    if submitted:
        if selected_strategy not in data: data[selected_strategy] = {}
        for ind in indicators:
            key_note = f"note__{sanitize_key(selected_strategy)}__{sanitize_key(ind)}"
            key_status = f"status__{sanitize_key(selected_strategy)}__{sanitize_key(ind)}"
            note_val = st.session_state.get(key_note,"")
            status_val = st.session_state.get(key_status,"Open")
            data[selected_strategy][ind] = {"note":note_val,"status":status_val,"momentum":strategy_type,"strategy_tag":strategy_tag,"analysis_date":analysis_date.strftime('%Y-%m-%d'),"last_modified":datetime.utcnow().isoformat()+"Z"}
        save_data(data)
...
strategies_to_show = daily_strategies if filter_strategy=="Today's Focus" else list(data.keys()) if filter_strategy=="All Strategies" else [filter_strategy] if filter_strategy in daily_strategies else list(data.keys())
for strat in strategies_to_show:
    st.markdown(f"### {strat}")
    inds = data.get(strat,{})
    if not inds: st.info("No saved notes for this strategy."); continue
    strategy_tag = next(iter(inds.values())).get("strategy_tag","Neutral")
    st.markdown(f"**Strategy Tag: {tag_colors.get(strategy_tag,strategy_tag)}**")
    st.markdown("---")
    for ind_name, meta in inds.items():
        momentum_type = meta.get("momentum","Not Defined")
        st.markdown(f"**{ind_name}** ({type_colors.get(momentum_type,momentum_type)})")
