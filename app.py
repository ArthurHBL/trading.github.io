...
def generate_csv_report(data, analysis_date):
    """Generate CSV report of all analyses"""
    report_data = []
...
STRATEGIES = {
 ...
 "RenkoVol": ["Overview","GR-MMAs","Pi Cycle","Keltner & Bollinger","RSI Ichimoku","RWI",
              "BBWP","Trend Master","VWAP-RSI","ASO"],
 "10h WWV Chart": ["Overview","BTC Log Regression","GR-MMAs","PiCycle","%R MA","RSI","Chaikin Oscillator",
                   "Ultimate HODL Wave","PVT","BBWP"],
 "Premium Osc Volatility": ["Overview","AO v2","ATR","RSI(ATR)","RSI","Stoch RSI"],
 "RSI Strategy": ["Overview","Supertrend","CM_Ultimate","FibZia","EVWMA_LB","RSI(63)","%R MA","WT_X",
                  "RAR [LuxAlgo]","RAINBOW_RSI","Coppock Curve","OBV","Global Liquidity Index"],
 "WeisWaveVol": ["Overview","Bitcoin Rainbow Wave","CVD Candles","Volume Delta","RSI(Volume Delta)","CMF","RAINBOW_RSI"],
 "PremiumACC": ["Overview","GC","PiCycle Top Indicator","EMA Ribbon","RCI3 Lines","TDIGM","CVD Ichimoku",
                "RSI(OSC)","RSI","Crypto Fear & Greed Index","BBWP"],
 "VolPress": ["Overview","Alligator","GR-MMAs","CVO","WWV","RWI","MACD","TSI","Wick Delta Buy/Sell Pressure",
              "Elasticity","WT_LB"],
 "Volatility": ["Overview","Symmetrical STD Channel","4 SMA","Golden Ratio Fib","RECON ATR","BBWP","SMI",
                "Stoch RSI","CA_Fisher","RSI","RSI line"],
 "ACC/DIST": ["Overview","5 SMMA","Demand Index","BTC Transaction Fees","Ratings","BBWP","MVRV Z-Score"],
 "LuxAlgo": ["Overview","Symmetrical STD Channel","Ultimate RSI","RWI"],
 "Point and Figure": ["Overview","RW Monte Carlo","CM SuperGuppy","AOv2","BBWP","SNAB_RSI_EMA",
                       "CM_Williams_Vix_Fix","ASO","RAINBOW_RSI"],
 "Rational Strategy LT": ["Overview","MMBs","GR-Multiple MAs","SAR","Support and Resistance","Coppock Curve",
...
with col2:
    if st.button("📊 CSV"):
        if data:
            csv_buffer = generate_csv_report(data, analysis_date)
            st.download_button(
                label="Download CSV",
                data=csv_buffer,
                file_name=f"trading_report_{analysis_date.strftime('%Y%m%d')}.csv",
                mime="text/csv"
            )
        else:
...
for i, strategy in enumerate(daily_strategies):
    ...
    if strategy in data and any(ind.get('analysis_date') == analysis_date.strftime('%Y-%m-%d') for ind in data[strategy].values()):
        col.success(f"✓ {strategy}")
    elif strategy == selected_strategy:
        col.info(f"📝 {strategy} (current)")
    else:
        col.warning(f"⏳ {strategy}")

st.markdown("---")

# -------------------------
# Notes Form (Grid Layout)
# -------------------------
with st.form("notes_form"):
    ...
    for i, ind in enumerate(indicators):
        ...
        with col[i % n_cols].expander(ind, expanded=False):
